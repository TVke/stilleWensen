...
@section('content')
	...
	<section class="pt-16 flex flex-wrap justify-center">
		<form action="{{ route('contact') }}" method="post" class="w-64">
            ...
            <input id="subject" name="subject" value="{{ old('subject') }}" class="rounded-none inputfield{{ $errors->has('subject')?" error":"" }}" required>
			...
			<input id="name" name="from_name" value="{{ old('from_name') }}" class="rounded-none inputfield{{ $errors->has('from_name')?" error":"" }}" required>
			...
			<input type="email" name="from_email" value="{{ old('from_email') }}" id="email" class="rounded-none inputfield{{ $errors->has('from_email')?" error":"" }}" required>
			...
			<input id="question" name="question" value="{{ old('question') }}" class="rounded-none inputfield{{ $errors->has('question')?" error":"" }}" required>
