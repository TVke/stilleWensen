...
@section('content')
	<form action="{{ route('store_wisher') }}" method="post">
		...
		<label for="allow_public" class="text-left block max-w-lg mx-auto p-8{{ $errors->has('allow_public')?" text-red":"" }}"><input type="checkbox" class="mr-2" name="allow_public" id="allow_public" {{ (old('allow_public'))?"checked":"" }}> {{ __('app.form-public') }}</label>
